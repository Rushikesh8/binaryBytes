...
class CustomAccountManager(BaseUserManager):
    ...
    def create_superuser(self, email, contact, password, **other_fields):
        ...
        return self.create_user(email, contact, password, **other_fields)

    def create_user(self, email, contact, password, **other_fields):

        if not email:
            raise ValueError(_('You must provide an email address'))

        email = self.normalize_email(email)
        user = self.model(email=email, contact=contact, **other_fields)
